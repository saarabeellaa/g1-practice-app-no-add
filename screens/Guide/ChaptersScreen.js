...
import { getInterstitialExtraChapterId } from '../../config/adConfig';
...
export function ChaptersScreen({ navigation }) {
  ...
  // Initialize interstitial ad for extra chapters
  const { showAd, loaded } = useInterstitialAd(getInterstitialExtraChapterId());
    ...
    <View style={{ flex: 1, backgroundColor: '#f5f5f5' }}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingTop: 8 }}>
        ...
        {topics.map((topic, index) => {
          ...
          // Determine if this is an extra chapter (chapters after the first 4 are "extra")
          // ADJUST THIS LOGIC based on your actual business rules
          const isExtraChapter = index >= 4;
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => {
                if (isExtraChapter) {
                  // Show interstitial ad before opening extra chapter
                  console.log('Opening extra chapter, showing ad...');
                  showAd(() => {
                    console.log('Ad closed, navigating to chapter');
            ...
            >
              ...
              {/* Premium Badge for Extra Chapters */}
              {isExtraChapter && (
                <View
                  style={{
                    position: 'absolute',
